 ...
 */
void parse_string(char const *rawdata,
                std::vector<double> &data,
                std::vector<int> &indices,
                std::vector<int> &indptr,
                std::vector<double> &labels)
{
  std::stringstream string_stream(rawdata);

  if (!string_stream)
    throw std::ios_base::failure("Cannot put raw data string into std::stringstream");
...
static PyObject *load_svmlight_string(PyObject *self, PyObject *args)
{
  try {
    ...
    if (!PyArg_ParseTuple(args, "y", &rawdata))
      return 0;

    std::vector<double> data, labels;
    std::vector<int> indices, indptr;
    parse_string(rawdata, data, indices, indptr, labels);
